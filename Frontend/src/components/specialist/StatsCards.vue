...
const accentClass = (item: StatCardItem) => {
  if (item.accent) return item.accent;
  if (item.iconColor.includes("amber")) return "from-amber-500 to-orange-500";
  if (item.iconColor.includes("green") || item.iconColor.includes("emerald")) return "from-emerald-500 to-teal-500";
  if (item.iconColor.includes("purple") || item.iconColor.includes("violet")) return "from-violet-500 to-purple-500";
  if (item.iconColor.includes("red")) return "from-red-500 to-rose-500";
...
<template>
  <div class="grid gap-4" :class="gridClass">
    <div
      v-for="item in items"
      :key="item.label"
      class="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md"
    >
      <div class="h-1.5 bg-gradient-to-r" :class="accentClass(item)" />
